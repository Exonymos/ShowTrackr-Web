# apps/desktop/tests/routes/test_main_routes.py
import pytest
from flask import url_for, session as flask_session
from src.core.models import WatchlistItem
from src.core import db as main_db, config
...
def test_index_page_loads(client):
    ...
    assert b"SeriesScape" in response.data  # Check for app name
    assert b"Welcome to SeriesScape!" in response.data
...
def test_load_watchlist_htmx(client, db_session):
    ...
    db_session.commit()
    response = client.get(url_for("main.load_watchlist"), headers={"HX-Request": "true"})
    assert response.status_code == 200
...
def test_load_watchlist_full_page_reload(client):
    ...
    assert response.status_code == 200
    assert b"SeriesScape" in response.data  # Full page structure
...
def test_about_page_loads(client):
    ...
    assert response.status_code == 200
    assert b"About SeriesScape" in response.data
...
def test_load_watchlist_sort_year_nulls_last(client, db_session):
    ...
    add_test_item(db_session, title="Year Null", year=None, date_watched=date(2023, 1, 1))
    add_test_item(db_session, title="Year 2022", year=2022, date_watched=date(2023, 1, 2))
    add_test_item(db_session, title="Year 2023", year=2023, date_watched=date(2023, 1, 3))
...
def test_500_error_page_on_index_route(client, app):
    ...
    with patch("src.core.routes.main.get_watchlist_data") as mock_get_data:
        mock_get_data.side_effect = Exception("Mocked 500 error from get_watchlist_data")
