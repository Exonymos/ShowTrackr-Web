# apps/desktop/tests/routes/test_settings_routes.py
import pytest
from flask import url_for, session, get_flashed_messages
from src.core import config
from src.core.models import WatchlistItem
from src.core import db as main_db
from src.core.routes.settings import allowed_file
import json
import io
...
def test_export_data_json(client, db_session):
    ...
    assert response.headers.get("Content-Disposition").startswith(
        "attachment; filename=seriesscape_backup_"
    )
    ...
    assert any(d["title"] == "Export Show 1" and d["rating"] == 7 for d in exported_data)
